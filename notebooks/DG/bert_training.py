...
from transformers.trainer import is_apex_available, get_tpu_sampler
...
class Trainer:
    ...
    def train(self, model_path: Optional[str] = None):
        ...
        class_weight = train_dataloader.dataset.class_weight
        ...
        print('{}{}{}'.format('self.arg is','\t',self.args.local_rank))
        if self.args.local_rank != -1:
        ...
        for epoch in train_iterator:
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                inputs['class_weight'] = class_weight
    ...
    def _prediction_loop(
            ...
            with torch.no_grad():
                ...
                if not prediction_loss_only:
                    if preds is None:
                        ...
                        input_label_ids = inputs["labels"].detach().cpu().numpy()
                    else:
                        preds = np.append(preds, logits.detach().cpu().numpy(), axis=0)
                        input_label_ids = np.append(input_label_ids, inputs["labels"].detach().cpu().numpy(), axis=0)

                #eval_loss = eval_losses / step_eval_loss
                ...
                for i in range(input_label_ids.shape[0]):
                    for j in range(input_label_ids.shape[1]):
                        #if out_label_ids[i, j] != pad_token_label_id:
                        #out_label_list[i].append(out_label_ids[i][j])
                        #preds_list[i].append(preds[i][j])
                        if input_label_ids[i][j] == -100:
                            continue

                        gold_label_list.append(input_label_ids[i][j])
                        preds_list.append(preds[i][j])

        accuracy = (np.array(gold_label_list) == np.array(preds_list)).mean()
        print(accuracy)

        print(confusion_matrix(np.array(gold_label_list),np.array(preds_list)))

        precision,recall,f1,support = score(gold_label_list, preds_list)
        print(precision)
        print(recall)
        print(f1)

        results = {
            "precision": precision_score(gold_label_list, preds_list,average='macro'),
            "recall": recall_score(gold_label_list, preds_list,average='macro'),
             "f1": f1_score(gold_label_list, preds_list,average='macro')
        }
        ...
        if self.compute_metrics is not None:
            if preds_list is not None and gold_label_list is not None:
                #preds = np.argmax(preds,axis=2)
                metrics["token_classification"] = self.compute_metrics(EvalPrediction(predictions=preds_list, label_ids=gold_label_list))
                metrics = self.compute_metrics(EvalPrediction(predictions=preds_list, label_ids=gold_label_list))
        else:
            metrics = {}
        ...
        return PredictionOutput(predictions=preds_list, label_ids=gold_label_list, metrics=metrics)